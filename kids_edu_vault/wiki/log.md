...
## [2026-09-19] correction | 경계가 증발한 문장 5건 정정 — "없다"의 범위
- Type: correction (선행 작업: 2026-09-19 두 ingest)
- ⭐ **내가 쓴 문장을 내가 반증했다.** [[chalk-instructor-field-reality]]는 §0에 **"이 볼트 문서 전수 조사"**라고 범위를 적어 놓고, **본문과 다른 페이지에서 그 경계를 빼고 단정**했다 — *"실시된 회차의 사후 현장 기록이 사실상 0건이다."*
- **틀렸다.** `hypeprooflab`에 **회차별 운영 기록이 있다**(운영 절차 3회 변경 이력 + 사고 5건). 출처는 **Router 전언이며 이 세션이 직접 확인하지 않았다.**
- 🔴 **보고할 때 내가 센 것보다 많았다** — Router에게는 "§4 한 줄"이라고 보고했으나, **실제로 세어 보니 다섯 군데**였고 **내가 인용한 그 문장(`세 회차 어느 것도…`)은 페이지에 없었다**(내 보고 메시지에만 있었다). **정정 대상을 정정 전에 세지 않은 것**이 또 하나의 같은 실수다
- 정정한 곳 5: [[chalk-instructor-field-reality]] §0 서두·표 헤더·표 3행·§0 결론·§4 제목 · [[chalk-entry-point-20260918]] §5 · [[index]] · [[intel/_index]]
- 남긴 곳 1: **`log.md`의 2026-09-18 엔트리는 고치지 않았다** — 볼트 규약이 *"log는 추가 전용: 과거 엔트리 수정 금지"*다. **이 엔트리가 그 정정이다**
- 방식: 지우지 않고 **`[!contradiction]` 정정 블록**으로 남겼다. 초판 문장을 인용하고 왜 틀렸는지 적었다 — CI 표의 *"닫힘 + PR 번호"*, [[verification-discipline]]의 철회 기록과 같은 규칙
- 표의 `현장 기록` 열을 **"이 볼트 / lab" 두 칸으로 갈랐다.** lab 쪽은 **전수 확인하지 않았으므로 "lab에 무엇이 얼마나 있는지"는 이 문서가 답하지 않는다**고 명시
- 📌 **[[verification-discipline]] 규범 1의 사례가 하나 늘었다** — *"「없다」가 아니라 「이 방법으로는 안 보인다」"*. 이번 것은 **같은 문서 안에서 경계가 증발한 형태**이고, 남이 아니라 **본인이 몇 줄 위에 정확히 써 둔 것**이 아래에서 넓어졌다
- 함께 커밋: 미커밋 상태였던 [[verification-discipline]] (규범 4 + 작동 기록 + 철회 기록)
- ⬜ **이번에 안 넣은 것** — lab 운영 기록의 **내용**(절차 3회 변경·사고 5건), 자격증명 체계, 리허설 인과 사슬, 넉 달 미결, PR 결과. **PR 다섯이 미머지라 기준 커밋을 모르고, 그러면 무효화 후보를 못 닫는다.** 다음 판이다
- Pages updated: [[chalk-instructor-field-reality]] · [[chalk-entry-point-20260918]] · [[verification-discipline]] · [[index]] · [[intel/_index]] · [[log]]

## [2026-09-19] ingest | 자동 검사 커버리지 전수 — 배선 규약이 두 벌이다
- Type: ingest (1 source, **외부 경로**)
- Source: `scratchpad/chalk-scan:ci-coverage-census.md` · 기준 `hypeproof-studio` main `ce95747`(열린 PR 제외)
- ⚠️ **다른 저장소 조사이며 이 세션이 직접 확인하지 않았다.** 다만 **세션 둘이 서로 안 보고 독립적으로 재서 숫자가 일치**했다(스크립트 기준 / 테스트 파일 기준)
- 판정: **`wiki/` 트리.** 제품 실행·품질 기록이다. 검증 규범 1건은 `concepts/`로 승격했다
- Pages created: [[chalk-ci-coverage-census]] (`validation/`) · [[verification-discipline]] (`concepts/`)
- ⭐ **남길 것은 숫자가 아니라 이 문장이다** — *"**같은 저장소에 시험 배선 규약이 두 벌이다.** 한쪽은 폴더를 통째로 집어 새 파일이 저절로 들어가고(누락 0), 다른 쪽은 이름을 하나씩 손으로 적어 빠뜨리면 조용히 안 돈다. **그리고 파일을 만드는 사람은 자기가 어느 쪽에 있는지 모른다.**"* 구멍은 결과이고 **규약이 두 벌인 것이 원인**이다
- ★ **구멍 ②가 ①보다 위험하다** — "워크플로에 적혀 있다"와 "내 PR에서 돈다"가 다르다. **`lesson-pedagogy.ts`가 `dental-reference.yml`의 `paths`에 없어 관문만 고치는 PR은 브라우저 저작 시험이 아예 돌지 않는다.** [[chalk-pedagogy-gate]]의 `#1115`가 걸리지 않고 지나간 것은 **그 PR이 `authoring.ts`도 함께 고쳤기 때문 — 운이었다**
- ★ **실제 피해 2건** — ① 관문을 넣으면서 손 배선 쪽 시험을 깨뜨렸고 **검사는 초록이었다. 열흘 뒤 우연히 발견됐다** ② 위 경로 사각지대
- 숫자: PR 반응 워크플로 **22개 중 7개** · **브라우저 층은 둘뿐이고 둘 다 경로 필터로 걸러진다** · 시험 묶음 **59개 중 28개** 미실행 · 서버 시험 파일 **67개 중 9개 체인 밖(6개는 아무 데서도 안 돎)** · `tests/rehearsal` **10개 전부**
- ⚠️ **`scrub-secrets.test.mjs`** — 코치 출력의 자격증명 유출을 막는 **가드의 유일한 테스트**인데 **스크립트조차 없다.** 손으로 돌리면 통과한다 → *깨져 있다는 뜻이 아니라 **깨져도 아무도 모른다**는 뜻*
- ⚠️ **이 표는 하한선이다** — 원본이 자기 방법의 한계를 6개 적었고, 그중 `continue-on-error` 미확인이 가장 크다. **"돈다"가 "막는다"를 뜻하지 않는다**
- ⬜ **미결 — 답을 지어내지 않았다**: **두 규약 중 어느 쪽으로 통일하는가.** 글롭은 고아를 없애지만 **실기기·실제 SDK처럼 실행 조건이 다른 시험까지 끌어들인다.** 위클리 제안서 §10에 올라갔다. 그 밖에 `tests/rehearsal` 성격 · `:d1` 여섯 중 셋만 넣은 것이 의도인가 · `paths`가 의도적으로 좁은가 · `extension / smoke`가 필수 컨텍스트인가(**403, GitHub API 한도 소진으로 확인 불가**)
- 🆕 **검증 규범 3개를 `concepts/`로 올렸다** ([[verification-discipline]]) — 하루에 **세 사람이 서로 다른 방향으로 같은 함정**에 걸렸다(안 도는 걸 돈다고 / 도는 걸 안 돈다고 / **옛 브랜치를 보고 "확인했다"고**). ① 부분만 보고 "없다" 단정 금지 ② **워킹트리가 아니라 푸시된 것**을 읽기 ③ **음성 결과 전에 양성 대조군**(꺼진 화면의 검은 스크린샷을 "대화상자 없음"으로 읽을 뻔했다)
- 💡 비용 0 수정 1순위: **`dental-reference.yml`의 `paths`에 빠진 파일 추가** — 목록 수정 한 번이면 그 시험이 실제로 지나는 경로를 덮는다
- Pages updated: [[chalk-pedagogy-gate]](§6에 사각지대 경고) · [[chalk-implementation-status]] · [[chalk-requirement-coverage-audit]] · [[chalk]](2026-09-19절) · [[concepts/_index]] · [[index]] · [[hot]] · [[log]] · `.raw/.manifest.json`
