...
## ⚠️ 2026-09-14 — "우리는 지금 아무도 안 쓴다"

[[2026-09-14-weekly-on-hypeproof]] §1. [[jay-lee]]가 미국 체류 중 직접 결제해 쓰면서 내린 진단이다.

> *"스튜디오를 우리는 지금 **아무도 안 쓴다.** 내가 스튜디오 새로 배포됐다고 했을 때 그거 써 보신 분? 100% 아무도 안 봤을 것 같은데."*
> *"UI가 똑같아서 먼저 바꾸기 시작했고, 개선하다 보니 옆에 계시던 상무님이 '나 이거 기업에 팔고 싶은데 써 볼 수 있냐'고 하시는데 **누군가 써 볼 수 있는 기능이 아무것도 안 돼 있어서** 체험판을 만들었어요."*
> *"그분은 성인도 전문직도 아닌데, 이 AI 도구를 통해 **뭔가를 깨닫게 해 줘야 되는데 그게 뭔지 나조차도 잘 모르겠더라고요.** (…) 체험판을 만들고 다 빌드했음에도 **우리가 원하는 어떤 것을 전달할 수 없었다.**"*

→ [[internal-productization]] 결정의 출발점. 직전 ingest에서 기록한 **현재 기능 6 / 로드맵 7**은 기능 목록이고, **실사용은 별개 문제**임이 확인됐다.

부수로 나온 것:
- [[jiwoong-kim]]이 **개발용 버전을 따로 배포**해 쓰고 있었다 (9/16 [[jeon-sangyeol]] 데모용)
- 그 데모에는 **문제 정의 · AI 위임 내용 · 밸리데이션 과정**이 드러나고, **로컬 로그 파일에 에셋 달성도가 저장**된다
- ⚠️ **HypeProof 코드를 Studio 레포에서 분리**하는 방향에 합의 (시점 미정). [[jay-lee]]는 *"HypeProof 기능이 **웹 브라우저 형태로 고객에게 제공되는 게 맞는지**"*에 의문을 제기하고 UX/UI 전문가 자문을 예고했다
